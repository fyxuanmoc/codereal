...
public class Help {
    public static void main(String[] args){
        ...
        Help h=new Help();
        while(sc.hasNext()){
            int xj_Num=sc.nextInt();
            int[] x=new int[xj_Num];
            int[] y=new int[xj_Num];
            for(int i=0;i<xj_Num;i++){
                x[i]=sc.nextInt();
            }
            for(int i=0;i<xj_Num;i++){
                y[i]=sc.nextInt();
            }
            int res=h.helpYi(xj_Num,x,y);
            System.out.println(res);
        }
    }

    private int helpYi(int xj_num, int[] x, int[] y) {
